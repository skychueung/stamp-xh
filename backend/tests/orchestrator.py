...
class _RuntimeQuotaMonitor:
    """Monitor disk usage while a model subprocess runs and terminate on violation."""

    def __init__(self, pid: int, run_dir: str, quota_bytes: int, max_files: int, interval: float = 2.0):
        self.pid = pid
    ...
    def _loop(self) -> None:
        while not self._stop.is_set():
            total_size, file_count = self._measure()
            if total_size > self.quota_bytes:
                self.violation = f"disk quota exceeded: {total_size} > {self.quota_bytes}"
                self._terminate()
                return
            if file_count > self.max_files:
                self.violation = f"max output files exceeded: {file_count} > {self.max_files}"
                self._terminate()
                return
...
class P33LOrchestrator:
    ...
    def run_job(self, job: P33LJob, input_payload: dict) -> P33LJob:
        ...
        try:
            ...
            # Monitor quota while the subprocess runs
            monitor = _RuntimeQuotaMonitor(
                pid=job.pid,
                run_dir=job.run_dir,
                quota_bytes=cfg.disk_quota_bytes,
            ...
            if monitor.violation and job.status not in ("timeout",):
                job.status = "failed"
                job.result = self._load_wrapper_result(job.run_dir) or {"exit_code": exit_code}
                job.result["quota_error"] = monitor.violation
