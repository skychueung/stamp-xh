...
class PepHARAdapter(BaseModelAdapter):
    ...
    def submit_dev_smoke(
        ...
        """P33U-D23 formal adapter contract entry for dev-only PepHAR smoke.

        Delegates to SmokeRunnerAdapter (D22 hardened path: D10-3 AnchorBasedSampler
        under CUDA_VISIBLE_DEVICES=1, cuda:0=GPU1, 3x12aa, fixed seed=12345 for
        reproducibility, full provenance — script_sha256 / config_sha256 /
        checkpoint_sha256 / env / seed recorded on the job). The default
        ``submit()`` inherited from BaseModelAdapter stays BLOCKED (P31C
        read-only); this is the additive dev-smoke contract method invoked by
        ``app.services.p33u.dev_smoke_dispatch.dispatch_dev_smoke``. GPU0 prod
        untouched. Never writes dev-smoke as Top4 / primary; never claims
        experimental validation; never invokes PPFlow.
        """
        from app.services.p33u.smoke_runner import SmokeRunnerAdapter
        SmokeRunnerAdapter().run_pephar_smoke(job, job_dir, seed=seed)
