...
class DiffPepBuilderAdapter(BaseModelAdapter):
    ...
    def submit_dev_smoke(
        ...
        """P33U-D23 formal adapter contract entry for dev-only DiffPepBuilder smoke.

        Delegates to SmokeRunnerAdapter (D22 hardened path: D10-2 run_inference.py
        via Hydra on cuda:1, 3x12aa, full provenance — script_sha256 /
        config_sha256 / checkpoint_sha256 / env / seed recorded on the job). The
        default ``submit()`` above stays BLOCKED (P31B read-only); this is the
        additive dev-smoke contract method invoked by
        ...
        SmokeRunnerAdapter().run_diffpepbuilder_smoke(job, job_dir, seed=seed)
