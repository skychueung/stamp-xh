...
from app.routers import p33l
from app.routers import p33s, p33t, p33u
from app.routers.demo import router as demo_router  # /api/v1/demo (read-only demo overview)
...
def create_app() -> FastAPI:
    ...
    app.include_router(p33l.router)
    app.include_router(p33s.router)  # /api/v1/p33s (P33S real-model GPU + scientific scoring)
    app.include_router(p33t.router)  # /api/v1/target-design (P33T result delivery center)
    app.include_router(p33u.router)  # /api/v1/p33u (P33U D20A dev run console)
    ...
    app.include_router(demo_router)   # /api/v1/demo (read-only demo overview, 止血 demo mainline)
